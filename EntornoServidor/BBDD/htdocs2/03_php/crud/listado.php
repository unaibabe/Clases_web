...
//          echo "<td>".$contacto['altas']."</td></tr>";
        ...
        // FORMA PRO
         echo "<td><input onclick='cambiarAlta(\"" . $contacto["nombre"] . "\", \"" . $contacto["apellidos"] . "\",";
         echo "this.checked";
         echo ");' type='checkbox' ";
         
         // CORRECCIÓN 1: Se añade el 'echo' para que el checkbox se marque visualmente
         echo ($contacto["altas"] ? "checked" : '');
...
<script>
	...
	async function cambiarAlta(nombre,apellido,alta) {
		const peticionApi = {"nombre":nombre, "apellidos":apellidos, "altas":alta};
		try{
